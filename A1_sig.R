...
plot(EN.test.sig$out[1:11000,5], type="l", ylab="N")
...
hist(EN.test.lam$out[10001:50000,5], xlab="N", freq=FALSE, main="")

Di <- -2*EN.10.15.2$out$ll

pD <- 0.5*var(EN.10.15.2$out[, "Di"])
S <- cbind(seq(xlims[1], xlims[2], M),
           seq(ylims[1], ylims[2], M))
dist <- e2dist1(S, X)
sigma <- mean(EN.10.15.2$out[10001:50000, "sigma"])
lam0 <- mean(EN.10.15.2$out[10001:50000, "lam0"])
psi <- mean(EN.10.15.2$out[10001:50000, "psi"])
lam <- t(lam0*exp(t(-(D*D))/(2*sigma*sigma)))
lamv <- colSums(lam.cand*w) 
ll <- sum(dpois(n, lamv, log=TRUE) )
Dbar <- -2*ll
dic <- Dbar + (2*pD)